...
#   MODEL             - Model name (default: FunAudioLLM/Fun-CosyVoice3-0.5B-2512)
#   PORT              - Server port (default: 8000)
#   BATCH_SIZE        - Per-stage ``max_num_seqs`` for both talker and code2wav (default: 1)
#   GPU_MEM_TALKER    - gpu_memory_utilization for talker stage (default: 0.4 at bs=1, else 0.3)
#   GPU_MEM_CODE2WAV  - gpu_memory_utilization for code2wav stage (default: 0.2 at bs=1, else 0.2)
#   HF_MODEL          - HuggingFace model path (default: same as MODEL)
#   HF_TOKENIZER      - HuggingFace tokenizer path (default: <MODEL>/CosyVoice-BlankEN)
...
DEFAULT_MEM_T=$([ "${BATCH_SIZE}" = "1" ] && echo "0.4" || echo "0.3")
DEFAULT_MEM_C="0.2"
GPU_MEM_TALKER="${GPU_MEM_TALKER:-${DEFAULT_MEM_T}}"
GPU_MEM_CODE2WAV="${GPU_MEM_CODE2WAV:-${DEFAULT_MEM_C}}"
NUM_WARMUPS="${NUM_WARMUPS:-3}"
DEPLOY_CONFIG="vllm_omni/deploy/cosyvoice3.yaml"
...
if [ "${RUN_HF}" = true ]; then
    ...
    HF_ARGS="--model ${HF_MODEL} --num-prompts ${NUM_PROMPTS} --num-warmups ${NUM_WARMUPS} --gpu-device ${GPU_DEVICE} --config-name hf_transformers --result-dir ${RESULT_DIR}"
    
    if [ -n "${HF_TOKENIZER}" ]; then
        HF_ARGS="${HF_ARGS} --tokenizer ${HF_TOKENIZER}"
    fi
    
    python "${SCRIPT_DIR}/transformers/bench_tts_hf.py" ${HF_ARGS}
...
if [ -n "${RESULT_FILES}" ]; then
    python "${SCRIPT_DIR}/plot_results.py" \
        ...
        --labels ${LABELS} \
        --output "${RESULT_DIR}/cosyvoice3_benchmark_${TIMESTAMP}.png"
fi
